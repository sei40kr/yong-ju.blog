...
Google File System は大量のデータを扱うための分散ファイルシステムである。
...
- **同期レプリケーション**
- **結果整合性**
- Google File System 自体にロック機能はない (後述の Chubby と組み合わせることで実現可能)
...
- タブレットのデータは Google File System 上に保存される。
  タブレットデータの冗長化は Google File System が担う。
...
1. クライアントは Chubby にルートタブレットを管理するタブレットサーバーを問い合わせる
1. クライアントはルートタブレットサーバーに、目的のキーが含まれるメタデータタブレットを管理するタブレットサーバーを問い合わせる
1. クライアントはメタデータタブレットサーバーに、目的のキーを含むタブレットを管理するタブレットサーバーを問い合わせる

#### 書き込み

1. タブレットサーバーは書き込みたいデータを Google File System 上のコミットログに保存する。
...
1. タブレットサーバーは、目的のキーに対応するデータを Google File System 上の SSTable から取得してメモリ上のキャッシュに書き込み、クライアントに返す。
...
1. 新しいタブレットサーバーは、Google File System 上の SSTable のインデックス部分を古いものから memtable に取り込む。
...
- **非同期レプリケーション**
- **結果整合性**
- バックアップはその Chubby セルとは別のデータセンターの Google File System に保存される。
  これは Google File System 自体が Chubby に依存しているためである。
